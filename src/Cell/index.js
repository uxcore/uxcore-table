  ...
  renderActionItems(column, rowData, mode) {
    ...
    arr.push(
      <a
        ...
        key="action"
        onClick={
          me.handleActionClick.bind(me, actions[0].callback.bind(me, rowData, me.props.root))
        }
      >
        {actions[0].render
          ? actions[0].render(actions[0].title, me.props.rowData)
          : actions[0].title}
      </a>
    );
    const menu = (
    ...
    arr.push(
      <i className="kuma-icon kuma-icon-triangle-down" key="icon" />
    );
    let dropdownOptions = {
      key: 'icon',
      overlay: menu,
